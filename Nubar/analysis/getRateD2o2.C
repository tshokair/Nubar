...
//begin
{
  ...
  ifstream runList;
  runList.open("runlistd2oleta.txt");
  numfiles=0;
  while(!runList.eof()){
  ...
  //make the histogram
  TH1F * h1= new TH1F("h1","Accidental rate as function of energy with FV cuts from 555-600 in D2O",30,3,6);
  ...
  TH1F * h6= new TH1F("h6","Accidental rate as function of energy for Nhit >20 and fv of 600",30,3,6);
  h1->SetLineColor(1);
  h2->SetLineColor(2);
  h3->SetLineColor(3);
  h4->SetLineColor(4);
  h5->SetLineColor(5);
  h6->SetLineColor(6);
  h1->SetYTitle("Rate of accidentals");
