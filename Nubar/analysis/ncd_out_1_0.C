 ...
 *  2/21 Modified to be an outside search.
 */
...
{ 
	...
	Float_t timeWindow=.100;
	Float_t distanceWindow=500;
	Float_t distanceWindowNCD=350;
	...
	runList.close();
	TH1F * h1= new TH1F("h1","NCD Position (Outside)",1300,0,1300);
	TH1F * h2= new TH1F("h2","PMT position(Outside)",1300,0,1300);
	TH1F * h3= new TH1F("h3","PMT energy (Outside)",150,0,15);
	h3->SetXTitle("reconstructed energy [MeV]");
	TH1F * h4= new TH1F("h4","NCD energy (Outside)",140,0,1.4);
	h4->SetXTitle("reconstructed energy [MeV]");
	TH1F * h5= new TH1F("h5","NCD-PMT Time Difference (Outside)",5000,0,.5);
	TH1F * h6= new TH1F("h6","DT Follower",5000,0,.5);
	TH1F * h7= new TH1F("h7","Distance Separation neutron-positron (Outside)",1300,0,1300);
	TH1F * h8= new TH1F("h8","DR ncd-ncd",1300,0,1300);
	TH1F * h9= new TH1F("h9","Primary ID",50000,0,50000);
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
		...
		nd = Td->GetEntries();
		cerr<<nd<<" events in file "<<k<<", pmt event count is "<<numPMT<<endl;
			...
			*/
			if (ncdstat==1 && nes>nesmin) {
				...
				while (l<j) {
					...
					if (ncdstat==0) {
						...
						if (TMath::Abs(timediff)<timeWindow) {
							...
							if (damnFlag2==1 && ene>emin && rf<outerVolume && rf>innerVolume && dr<distanceWindow) {
								//cerr<<ene<<endl;
				...
				if (numPred[numPi]>1 || numFN[numPi]>1) {
					cerr<<"Multi-fold: "<<tID<<" had "<<numPred[numPi]<<" positron like predecessors and "<<numFN[numPi]<<" neutron like followers"<<endl;
					numPMTPMT+=numPred[numPi];
					h9->Fill(tID);
				}
				*/
				if (numPred[numPi]==1 || numFN[numPi]==1) {
					...
					if (numPred[numPi]==1 && numFN[numPi]==1) {
						...
						numNe++;
						h9->Fill(tID);
						//cerr<<"2-fold: "<<peID<<" "<<tID<<endl;
						//cerr<<"At times: "<<peTime<<" "<<piTime<<endl;
		...
		delete Td;
		delete f1;
		cerr<<"next ntuple "<<k<<endl;
		//end loop over ntuples

		}
		cerr<<numNCD<<" ncd events and "<<numPMT<<" - "<<numPMTPMT<<" pmt events"<<endl;
