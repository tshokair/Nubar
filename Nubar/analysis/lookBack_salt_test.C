...
{ 
	...
	runList.close();
	TH1F * h1= new TH1F("h1","Time Differences Salt",300,0,.3);
	...
	TH1F * h7= new TH1F("h7","Time Differences Acc Salt",600,-.3,.3);
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (cleanFlag1==1 ){
				...
				while (l<=j) {
					...
					if (TMath::Abs(timediff)<TMath::Abs(timeWindow)) {
