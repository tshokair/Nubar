...
{ 
	...
	//runList.open("NCDfissNCD.txt");
	runList.open("NCDreactor.txt");
	numfiles =0;
	ofstream cList;
	cList.open("cListNout.txt");
	ofstream tCoins;
	tCoins.open("tCoinsNout.txt");
	while(!runList.eof()){
	...
	TH1F * h7= new TH1F("h7","Distance Separation neutron-positron",375,0,1300);
	...
	TH1F * h13= new TH1F("h13","Egen Neutron",100,0,10.0);
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
			...
			if (ncdFlag==1) {
				...
				while (l<j) {
					...
					if (ncdstat==0 && thetaij>thetamin && thetaij<thetamax && itr>itrmin && itr<itrmax && beta14>betamin && beta14<betamax) {
						...
						if (TMath::Abs(timediff)<timeWindow && ene>emin && ene<emax && rf<outerVolume && rf>innerVolume) {
							h10->Fill(beta14);
							h11->Fill(thetaij);
							h12->Fill(itr);
							h5->Fill(timediff);
				...
				if (numPred[numPi]==1 || numFN[numPi]==1) {
					...
					if (numPred[numPi]==1 && numFN[numPi]==0) {
						numNe++;
						cList<<"2-fold"<<endl;
		...
		//h5->Fit("expo","","",0,.5);
		TF1 *fDoubleExp= new TF1("fDoubleExp","expo(0*expo(2))",0,.5);
		//h1->Fit("expo","","",0,.5);
		//h5->Fit("fDoubleExp","","",0,.5);
