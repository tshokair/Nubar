...
{ 
	...
	TH1F * h9= new TH1F("h9","PMT Energy",150,0,15);
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (ncdstat==0) {
				...
				if (ene>3.211 && rf>innerVolume && rf<outerVolume) {
					h9->Fill(ene);
				}
			...
			if (ncdstat==1 && nes>nesmin && nes<nesmax) {
				...
				while (m<=lbEnd) {
					...
					if (ncdstat==1)) {
						...
						if(TMath::Abs(timediff)<timeWindow && nes>nesmin && nes<nesmax ){
							//cerr<<nes_str<<endl;
							int index;
							if (nes_str!=-999999) {
								index=nes_str;
