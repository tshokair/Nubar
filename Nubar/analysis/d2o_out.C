 ...
 *  4/20 Updated code to account for energy difference
 *	5/15 fixed bug that overcounts two-fold
 */
...
{ 
	...
	Float_t itrmin= 0.55;
	Float_t itrmax=0.95;
	Float_t emin=5.0 +.511;
	Float_t ethresh=4.0+.511;
	Float_t outerVolume=650;
	Float_t innerVolume=550;

	//readin the file names
	ifstream runList;
	runList.open("reactorD2O.txt");
	//runList.open("b8InD2O.txt");
	numfiles =0;
	while(!runList.eof()){
	...
	TH1F * h10= new TH1F("h10","Positron Gen Energy",100,0,20);
	TH1F * h11= new TH1F("h11","Pred Nhits",250,0,250);
	TH1F * h12= new TH1F("h12","Neutron Energy",100,0,20);
	TH1F * h13= new TH1F("h13","e+e- Energy",100,0,20);
	TH1F * h14= new TH1F("h14","Neutron Rf",650,0,650);
	TH1F * h15= new TH1F("h15","e+e- Rf",650,0,650);
	TH1F * h16= new TH1F("h16","Candidate (R/Rav)^3",150,0,1.5);
	TH1F * h17= new TH1F("h17","Candidate Z",130,-650,650);
	...
	TH2F * p2= new TH2F("p2"," Positron Egen vs. Nhit",100,0,10,100,0,100);
	TH2F * p3= new TH2F("p3"," R vs. deltaR",55,595,650,100,0,500);
	TH2F * p4= new TH2F("p4"," R vs. deltaT",55,595,650,50,0,.150);
	p3->SetXTitle("Rf [cm]");
	p3->SetYTitle("Delta T [s]");
	p4->SetXTitle("Rf [cm]");
	p4->SetYTitle("Delta R [cm]");
	p->SetXTitle("Delta R [cm]");
	p->SetYTitle("Delta T [s]");
	h->SetXTitle("Primary Energy [MeV]");
	h->SetYTitle("Predecessor Energy [MeV]");
	h16->SetXTitle("(R/Rav)^3");
	h17->SetXTitle("Reconstructed Z-position [cm]");
	h2->SetLineColor(7);
	h3->SetLineColor(6);
	//graph points
	Double_t dt[100000];
			...
			if (cleanFlag1==1 ){
				...
				while (l<=lbEnd) {
					...
					if (TMath::Abs(timediff)<timeWindow) {
						...
						if ( ene>ethresh && rf<outerVolume && rf>innerVolume && itr<itrmax && itr>itrmin &&beta14<betamax && beta14>betamin && damnFlag2==1 ) {
				...
				if(checkR==1){
					...
					if (numPred[numCoin]==0) {
						...
						if (deltaR[0]<distanceWindow) {
							...
							h7->Fill(predR[0]);
							h7->Fill(predR[1]);
							p3->Fill(predR[1],deltaR[0]);
							p4->Fill(predR[1],predDt[1]);
						...
						//case d
							...
							h7->Fill(predR[0]);
							h7->Fill(predR[2]);
							p3->Fill(predR[2],deltaR[1]);
							p4->Fill(predR[2],predDt[2]);
							dt[nGr]=predDt[2];
							dr[nGr]=deltaR[1];
							nGr++;
						}
