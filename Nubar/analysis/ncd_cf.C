...
{ 
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if((Int_t(rdamn1)& 0xC444)==0 && (Int_t(rdamn2)& 0x0001)==0){
			    damnFlag0 =1;  
			}
			else {
			    damnFlag0=0;
			...
			if (ncdstat==1 && nes>nesmin && nes<nesmax && damnFlag0==1 && damnFlag1==1) {
				h13->Fill(egen);
				h1->Fill(rf);
				numNCD++;
				...
				while (l<j) {
					...
					if (ncdstat==0 && thetaij>thetamin && thetaij<thetamax && itr>itrmin && itr<itrmax && beta14>betamin && beta14<betamax) {
						...
						if (TMath::Abs(timediff)<timeWindow && ene>emin && ene<emax && rf<fiducialVolume && damnFlag==1) {
							h10->Fill(beta14);
							h11->Fill(thetaij);
							h12->Fill(itr);
							h5->Fill(timediff);
				...
				while (m<=lbEnd) {
					...
					if (ncdstat==1)) {
						...
						if(TMath::Abs(timediff)<timeWindow && nes>nesmin && nes<nesmax && damnFlag0==1 && damnFlag1==1 ){
							//cerr<<nes_str<<endl;
							int index;
							if (nes_str!=-999999) {
								index=nes_str;
				...
				if (numPred[numPi] + numFN[numPi]>2) {
					cerr<<1+numPred[numPi] + numFN[numPi]<<"-fold event"<<endl;
				}
