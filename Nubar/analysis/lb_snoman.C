 ...
 */
{
	...
	Int_t cutFlag;
	Int_t evNumLast;
	Int_t coin_events[1000000][10]={0.0};
	Float_t timeOfLast;
	Int_t l;
	Int_t lLast;
	Int_t neutronFlag;
	...
	TH1F * h2= new TH1F("h2","time",100,0,.1);
	...
	TH1F * h8= new TH1F("h8","",350,0,350);
	TH1F * h9= new TH1F("h9","Acceptance Ratios ",350,0,350);
	TH1F * h10= new TH1F("h10","Acceptance Ratios ",350,0,350);
	h8->SetLineColor(9);
	h8->SetLineWidth(3);
	h9->SetLineColor(2);
	h9->SetLineWidth(3);
    ...
    {	
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (cutFlag==1) {
				...
				while(l<=j) { //lookback at events
					...
					if (timediff<timeWindow &&damnflag2==1 && ene>ethresh &&(rf<avIn || (rf>avOut && rf<outerVolume))) {
						if (numPred[nump]==0) {
							posEn=ene;
						}
		...
		cap<<"Acceptance Ratios for Coincidences ["<<total2fold+total3fold<<" MC events total]";
		TString st=cap.str();
