...
{ 
    ...
    {	
		...
		for(Int_t tr=0; tr<numTrig; ++tr){ //loop around triggered source events
			for (int j=trig[tr]; j<trig[tr]+20; j++) {//loop over events

			Td->GetEntry(j);
			
			//cerr<<"looking at event "<<trig[j]<<endl;
		...
		}//end loop around source triggers
		delete Td;
		delete f1;
