...
{ 
	...
	runList.close();
	TH1F * h1= new TH1F("h1","Time Differences Salt (Outside)",500,0,.5);
	...
	TH1F * h10= new TH1F("h10","Follower Energy",100,0,20);
	TH1F * h11= new TH1F("h11","Follower Energy",100,0,20);

	TH2F * h= new TH2F("h"," Primary Energy vs Predecessor Enegry",100,0,10,100,0,10);
	TH2F * p= new TH2F("p"," Predessor Time Separation vs Predecessor Space Separation Salt (Outside)",500,0,500,100,0,1);
			...
			if (cleanFlag1==1 ){
				...
				while (l<=lbEnd) {
					...
					if (TMath::Abs(timediff)<timeWindow  && xe==xeI && ye==yeI && ze==zeI) {
						...
						//check if event is clean
						//&& itr<itrmax && itr>itrmin &&beta14<betamax && beta14>betamin && damnFlag2==1 
						if ( ene>ethresh && rf<outerVolume && itr<itrmax && itr>itrmin &&beta14<betamax && beta14>betamin && damnFlag2==1 ) {
							//cerr<<"pred found for ev "<<tempID<<" it was ev "<<evid<<" at time "<<(ut1)+(ut2)/1.E6+(ut3)/1.E9<<" time diff is "<<timediff<<endl;
							rsep=TMath::Sqrt((xfit-xi)*(xfit-xi)+(yfit-yi)*(yfit-yi)+(zfit-zi)*zfit-zi));
							if (rsep<distanceWindow) {
								numF[numCoin]++;
								lastClean=l;
								h10->Fill(ene);
								h11->Fill(tEn);
								//cerr<<"follower neutron"<<endl;
							}
							//h3->Fill(ene);
				...
				if (numF[numCoin]+numPred[numCoin]>1) {
					cerr<<numPred[numCoin]<<" preds, and "<<numF[numCoin]<<" followers"<<endl;
				}
				//lbEnd=l;
				//lastClean=lbEnd-1;
				//last check to see if the coincidences make the rcut
				if(checkR==1 && numF[numCoin]==0){
					twoFlag=0;
					threeFlag=0;
					if (numPred[numCoin]==0) {
						twoFlag=0;
	...
	h4->Draw();
	h.Draw("colz");
	TF1 *fDoubleExp= new TF1("fDoubleExp","expo(0)+expo(2)",0,.05);
	fDoubleExp->SetLineColor(3);
	h1->Fit("expo","","",0,.1);
	h1->Fit("fDoubleExp","","",0,.1);
