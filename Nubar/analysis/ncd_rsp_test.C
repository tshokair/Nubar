...
{ 
	...
	runList.close();
	TH1F * h1= new TH1F("h1","RSP Enengy vs. Position",1200,0,1200);
	TH1F * h2= new TH1F("h2","RSP-old Energy vs. position",1200,0,1200);
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
		...
		Td->SetBranchAddress("Enerou",&eneu);
		...
		leg->AddEntry(h1,"RSP", "l");
		leg->AddEntry(h2,"Old RSP", "l");
		leg->Draw();
