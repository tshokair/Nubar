...
{ 
	...
	//runList.open("ncd_partial.runlist");
	...
	TH1F * h19= new TH1F("h19","Multiplicity",100,0,100);
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (ncdFlag==1 ) {
				...
				if(nes_str!=-999999 && nes_str>=0 && nes_str<=39){
					xi=stringX[int(nes_str)];
					yi=stringY[int(nes_str)];
					tempSt=nes_str;
				}
				else if(mux_str!=-999999&& mux_str>=0 && mux_str<=39){
				...
				h19->Fill(numPred[numPi]+numFN[numPi]+1);
