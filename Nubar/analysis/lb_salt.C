 ...
 *  Modified 6/2/11
 ...
 11/02/11 Change the code so it is a mock analysis on simulated data. 
 */
{
			...
			if (cutFlag==1) {
				...
				while(l<=j) { //lookback at events
					...
					if (timediff<timeWindow &&timediff!=0&&damnflag2==1 && ene>ethresh &&rf<outerVolume) {
						...
						if (numPred[nump]==1) {
							cerr<<tid<<" occured at time "<<totalTime<<" and has a predecessors ev: "<<evid<<" at time "<<(ut1)+(ut2)/1.E6+(ut3)/1.E9<<" "<<timediff<<" later and "<<lastID <<" at time "<<lastTime<<endl;
							cerr<<tid<<" "<<totalTime<<" "<<temp_pos<<endl;
							cerr<<evid<<" "<<(ut1)+(ut2)/1.E6+(ut3)/1.E9<<" "<<nhits<<endl;
							cerr<<lastID<<" "<<lastTime<<" "<<lastNhit<<endl;
						}
						//cerr<<tid<<" occured at time "<<totalTime<<" and has a predecessor ev: "<<evid<<" at time "<<ut1<<" "<<timediff<<" later"<<endl;
						lastTime=(ut1)+(ut2)/1.E6+(ut3)/1.E9;
						lastID=evid;
						lastNhit=nhits;
				...
				if(checkR==1){
					if (numPred[nump]==0) {
						...
						if(deltaR[0]<distanceWindow &&deltaR[1]<distanceWindow &&deltaR[2]<distanceWindow){
							coin_events[nump][0]=numPred[nump];
						}
						else if (deltaR[0]<distanceWindow &&deltaR[1]<distanceWindow &&deltaR[2]>distanceWindow ){
							coin_events[nump][0]=numPred[nump];
						}
						else if (deltaR[0]<distanceWindow &&deltaR[1]>distanceWindow &&deltaR[2]>distanceWindow ){
							coin_events[nump][0]=numPred[nump]-1;
						}
						else if (deltaR[0]>distanceWindow &&deltaR[1]>distanceWindow &&deltaR[2]<distanceWindow ){
							coin_events[nump][0]=numPred[nump]-2;
						}
						else if (deltaR[0]>distanceWindow &&deltaR[1]>distanceWindow &&deltaR[2]>distanceWindow ){
							coin_events[nump][0]=numPred[nump]-2;
						}
