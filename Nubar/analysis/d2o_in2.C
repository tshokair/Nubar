...
{ 
	...
	Int_t numOnePred=0;
	Int_t numTwoPred=0;
	Int_t twoFoldEn[15]={0.0};
	Int_t threeFoldEn[15]={0.0};
	//flags
	Int_t damnFlag1=0;
	Int_t damnFlag2=0;
	Int_t cleanFlag1=0;
	...
	ofstream cList;
	cList.open("cListDin.txt");
	ofstream tCoins;
	tCoins.open("tCoinsDin.txt");
	ifstream runList;
	...
	TH1F * h14= new TH1F("h14","Candidate Z",130,-650,650);
    ...
    {	
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if((Int_t(rdamn1)& 0x04F7)==0 && (Int_t(rdamn2)& 0x6FE1)==0 && fratiBit[j]==0){
				damnFlag1 =1;  
			}
			else {
				damnFlag1=1;
			...
			if (rf<fiducialVolume  && damnFlag1==1 &&ene>emin&& itr<itrmax && itr>itrmin &&thetaij<thetamax && thetaij>thetamin) {
				cleanFlag1=1;
			}
			else {
				cleanFlag1=0;
			...
			if (cleanFlag1==1 ){
				...
				while (l<=lbEnd) {
					...
					if (TMath::Abs(timediff)<timeWindow) {
						...
						if((Int_t(rdamn1)& 0x04F7)==0 && (Int_t(rdamn2)& 0x6FE1)==0 && fratiBit[l]==0){
							damnFlag2 =1;  
						}
						else {
							damnFlag2=1;
						...
						if ( ene>ethresh && rf<fiducialVolume&& itr<itrmax && itr>itrmin &&thetaij<thetamax && thetaij>thetamin && damnFlag2==1 ) {
							...
							predBeta[numPred[numCoin]+1]=thetaij;
							checkR=1;
							numPred[numCoin]++;
							lastClean=l;
							h1->Fill(timediff);
				...
				if(checkR==1){
					...
					if (numPred[numCoin]==0) {
						...
						if (deltaR[0]<distanceWindow) {
							...
							cList<<" Neutrino Energy: "<<pEnu<<endl;
							cList<<"EVID Energy Time (x,y,z) Nhit Thetaij ITR"<<endl;
						...
						if(deltaR[0]<distanceWindow &&deltaR[1]<distanceWindow &&deltaR[2]<distanceWindow){
							...
							cList<<"dt1= "<<predDt[1]<<" dt2:"<<predDt[2]<<" r0= "<<TMath::Sqrt(predX[0]*predX[0]+predY[0]*predY[0]+predZ[0]*predZ[0])<<" r1= "<<TMath::Sqrt(predX[1]*predX[1]+predY[1]*predY[1]+predZ[1]*predZ[1])<<" r3:"<<TMath::Sqrt(predX[2]*predX[2]+predY[2]*predY[2]+predZ[2]*predZ[2])<<" dr1= "<<deltaR[0]<<" dr2:"<<deltaR[1]<<" dr3:"<<deltaR[2]<<endl;
							h4->Fill(predID[0]);
							h4->Fill(predID[1]);
							h4->Fill(predID[2]);
							h12->Fill(jdy_i);
		...
		cList.close();
		tCoins.close();
		for (int nm=4; nm<15; nm++) {
			cerr<<"For nu energy:"<<nm<<" there were "<<twoFoldEn[nm]<<" 2-fold events and "<<threeFoldEn[nm]<<" 3-fold events."<<endl;
		}
