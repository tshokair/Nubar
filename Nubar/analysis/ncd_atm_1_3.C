...
{ 
	...
	runList.close();
	ofstream cList;
	cList.open("cListNatm.txt");
	ofstream tCoins;
	tCoins.open("tCoinsNatm.txt");
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (ncdFlag==1 ) {
				...
				if (numPred[numPi] + numFN[numPi]>2) {
					...
					cList<<numPred[numPi]<<"(pmt)+"<<numFN[numPi]+1<<"(ncd)-fold event"<<endl;
