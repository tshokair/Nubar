...
{//start code
	...
	int t;
	/*
	 for (Int_t f=0;f<nd;f++){
	 Td->GetEntry(f);
	 if(frati!= fratiBit[f]){
	 t=f;
	 Td->GetEntry(t);
	 while(frati!=fratiBit[t]){
	 cerr<<t<<" "<<evid<<" "<<nhits<<" "<<frati<<" "<<dt0<<endl;
	 t++;
	 }
	 }
	 }
		...
		if(cleanFlag==1 && dt0==99999){
			...
			do{
				...
				if( rf<FV && ene>etrig && hlcFlag==1 && damnFlag2==1 &&nhits<nhitmax ){
					 ...
					 */
					numClean++;
					dR=TMath::Sqrt((xfit-xi)*(xfit-xi)+(yfit-yi)*(yfit-yi)+(zfit-zi)*(zfit-zi));
					dR2=TMath::Sqrt((xfit-xi)*(xfit-xi)+(yfit-yi)*(yfit-yi));
					//h1->Fill(dR);
					//h9->Fill(dR2);
					if(dt0<timeWindow ){
						h1->Fill(dR);
			...
			for(int pr=0;pr<preds;pr++){
				h14->Fill(PeEn[coinNum][pr]);
				if (PeEn[coinNum][pr]>emin){
					piFlag=1;
				}
			}
