//#include <string>
{
	...
	files[0]="mc_b8nc_015997_p1.root";
	...
	for (Int_t k=0; k<numfiles;++k)
    {
		...
		for(Int_t j=0; j<nd; ++j){
			...
			day_diff = jdy-jdy_i;
			totalTime_i=ut1_i+ut2_i/1.E6-ut3_i/1.E9;
			...
			if (nhits>10 &&nhits<150 &&rmag<550&&day_diff==0 &&thetaij>.75 &&thetaij<1.45&&itr<.95&&itr>.55 &&damnflag==1){
				numev++;
				h1->Fill(nhits);
				h10->Fill(enep);
				totalTime=Int_t(ut1)+Int_t(ut2)/1.E6-Int_t(ut3)/1.E9;
				timediff=totalTime-totalTime_i;
				if (timediff<.2) {
					//cerr<<timediff<<endl;
				}
				//dt1=ut1-ut1_i;
				...
				if((isFirst==0 && isSecond==0)||isThird==1||rfit>350||timediff>.2){
					tempEventID=evid;
					temp_pos=enep;
					ut1_i=ut1;
					ut2_i=ut2;
				...
				else if ( /*is_pos==0 &&*/ rfit<350 &&timediff<.150){
