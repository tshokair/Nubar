 ...
 *  but the analysis code was fixed to make sure no coincidences occured with different event numbers and the same gtid)
	12/14/11 Changed code to include high level cuts 
 */
			...
			if (cleanFlag1==1 ){
				...
				while (l<=j) {
					...
					if (timediff<timeWindow && timediff!=0) {
						...
						if (damnFlag2==1 && ene>ethresh && rf<fiducialVolume&& itr<itrmax && itr>itrmin &&thetaij<thetamax && thetaij>thetamin) {
							//cerr<<"pred found for ev "<<tID<<" it was ev "<<evid<<" at time "<<(ut1)+(ut2)/1.E6+(ut3)/1.E9<<" time diff is "<<timediff<<endl;
