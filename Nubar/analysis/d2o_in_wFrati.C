...
{ 
    ...
    {	
		...
		for(Int_t j=0; j<nd; ++j){
			Td->GetEntry(j);
			if (fratiBit[j]==1) {
				cerr<<evid<<" is frati follower"<<endl;
			}
			...
			if((Int_t(rdamn1)& 0x01b5)==0 && (Int_t(rdamn2)& 0x6FE1)==0 && fratiBit[j]==0){
				damnFlag1 =1;  
			}
			else {
				damnFlag1=0;
			...
			if (cleanFlag1==1 ){
				...
				while (l<=lbEnd) {
					...
					if (TMath::Abs(timediff)<timeWindow) {
						...
						if((Int_t(rdamn1)& 0x01b5)==0 && (Int_t(rdamn2)& 0x6FE1)==0 && fratiBit[l]==0){
							damnFlag2 =1;  
						}
						else {
							damnFlag2=0;
