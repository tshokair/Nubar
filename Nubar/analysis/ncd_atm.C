...
{ 
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (ncdstat==1 && nes>nesmin && nes<nesmax && fratiBit[j]==0) {
				//h13->Fill(egen);
				h1->Fill(rf);
				numNCD++;
				...
				while (l<j) {
					...
					if (ncdstat==0 && thetaij>thetamin && thetaij<thetamax && itr>itrmin && itr<itrmax && beta14>betamin && beta14<betamax && fratiBit[l]==0) {
				...
				while (m<=lbEnd) {
					...
					if (ncdstat==1)) {
						...
						if(TMath::Abs(timediff)<timeWindow && nes>nesmin && nes<nesmax && fratiBit[m]==0 ){
							//cerr<<nes_str<<endl;
							int index;
							if (nes_str!=-999999) {
								index=nes_str;
