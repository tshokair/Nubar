...
{ 
	...
	//runList.open("alphaD2Oc.txt");
	//runList.open("atm_d2o.runlist");
	numfiles =0;
	while(!runList.eof()){
	...
	TH1F * h12= new TH1F("h12","Theta ij D2O",1000,0,10);
	TH1F * h13= new TH1F("h13","In Time Ratio D2O",1000,0,10);

	TH1F * h14= new TH1F("h14","In Time Ratio D2O",650,0,650);
	TH1F * h15= new TH1F("h15","In Time Ratio D2O",650,0,650);
	h12->SetXTitle("Theta ij");
	h13->SetXTitle("ITR");
    ...
    {	
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (rf<fiducialVolume  && damnFlag1==1 &&ene>emin&& itr<itrmax && itr>itrmin && thetaij>thetamin && thetaij<thetamax) {
				cleanFlag1=1;
			}
			else {
				cleanFlag1=0;
	...
	TCanvas *c2 = new TCanvas("c2","Root Canvas 2"); // Default Canvas 
		h3->DrawNormalized();
		h2->DrawNormalized("same");
		leg= new TLegend(0.6,0.7,0.89,0.89);
		leg->AddEntry(h2,"Neutrons", "l");
		leg->AddEntry(h3,"Positrons ", "l");
		leg->Draw();
