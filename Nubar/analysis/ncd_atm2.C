...
{ 
	...
	//runList.open("NCDreactorAcr.txt");
	//runList.open("NCDreactorIn.txt");
	runList.open("atmN.txt");	
	numfiles =0;
	while(!runList.eof()){
	...
	for (Int_t k=0; k<(numfiles-1);++k){//loop over ntuples/root files
		...
		for(Int_t m=0; m<nd; m++){
			...
			if(nhits>nhitMax ){
				...
				while(n<endM ){
					...
					//cerr<<tdiff<<endl;
					if (TMath::Abs(tdiff)<=.250 ){
						fratiBit[n]=1;
						fratiFlag=1;
						numMMF++;
						lastMMF=n;
						// cerr<<lastM<<" "<<burstEvent<<" frati "<<endl;
					}
					n++;
				}
		...
		//nhit burst cut
		int burstBit[5000000]={0.0};
    int burstID[100];
    int totalBurst=0;
    for(Int_t p=0; p<nd; p++){
      nhitBurst=0;
      Td->GetEntry(p);
      if(nhits>40 && ncdstat==0 && p<nd ){
	int nb=p+1;
	double burstTime=0;
	Float_t burstStart=ut1;
	int burstNum=0;
	int endBurst=0;
	int burstDay=jdy;
	double bTime=0;
      
	while(burstTime<4 && nb<nd){
	  Td->GetEntry(nb);
	  if(Int_t(jdy)-Int_t(burstDay)==0){
	    burstTime=ut1-burstStart;
	  }
	  else if (Int_t(jdy)-Int_t(burstDay) == 1 ){
	    burstTime=ut1+(86400-burstStart);
	  }
	  else burstTime=5;
	  //cerr<<burstTime<<endl;
	  if((burstTime <4 && burstTime>=0) && (nhits>40 || ncdstat==1)){
	    burstID[burstNum]=nb;
	    burstNum++;
	  }
	  if(burstNum>=5 && burstTime<=4 && burstTime>0){
	    nhitBurst=1;
	    bTime=burstTime;
	    endBurst=nb;
	    //cerr<<"nhit burst "<<burstNum<<"-fold burst in "<<burstTime<<" s"<<endl;
	  }
	  nb++;
	}
	if(nhitBurst==1){
	  totalBurst++;
	  for(int q=0;q<burstNum;q++){
	    burstBit[burstID[q]]=1;
	  }
	  //cerr<<"nhit burst "<<burstNum<<"-fold burst in "<<bTime<<" s"<<endl;
	}
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (ncdstat==1 && nes>nesmin && nes<nesmax && fratiBit[j]==0 && burstBit[j]==0) {
				//h13->Fill(egen);
				h1->Fill(rf);
				numNCD++;
				...
				while (l<j) {
					...
					if (ncdstat==0 && thetaij>thetamin && thetaij<thetamax && itr>itrmin && itr<itrmax && beta14>betamin && beta14<betamax && fratiBit[l]==0 && burstBit[l]==0) {
				...
				while (m<=lbEnd) {
					...
					if (ncdstat==1)) {
						...
						if(TMath::Abs(timediff)<timeWindow && nes>nesmin && nes<nesmax && fratiBit[m]==0 && burstBit[m]==0 ){
							//cerr<<nes_str<<endl;
							int index;
							if (nes_str!=-999999) {
								index=nes_str;
