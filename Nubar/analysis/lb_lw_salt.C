...
//start code
{
	...
	Float_t predX[1000]={0.0};
	Float_t predY[1000]={0.0};
	Float_t predZ[1000]={0.0};
	Float_t predR[1000]={0.0};
	Float_t deltaR[3];
    ...
    {	
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if(rf<outerVolume && damnflag==1 && ene>ethresh){
				//cutFlag=0;
				totalClean++;
				neutronFlag=1;
			...
			if (cutFlag==1){
				...
				while (l<j) {
					...
					if (timediff<timeWindow && rf<outerVolume && rf>innerVolume && ene>ethresh) {
				...
				if (numPred[numN]>0) {
					...
					if(numPred[numN]==1){
						...
						cerr<<"delta r= "<<deltaR[0]<<" cm from rf ="<<ri<<" to ri= "<<predR[0]<<" in a time of "<<deltaT<<endl;
						h1->Fill(deltaR[0]);
					}
				
					else if(numPred[numN]>1){
						cerr<<evNumLast<<endl;
					}
					*/
				}
			...
			if (neutronFlag==1){
				...
				while (l<j) {
					...
					if (timediff<timeWindow+2 && ene>ethresh) {
						rfit=TMath::Sqrt((xfit-xi)*(xfit-xi)+(yfit-yi)*(yfit-yi)+(zfit-zi)*(zfit-zi));
						preds++;	
						h2->Fill(rfit);
					}
