...
//paramters
  ...
  TFile *f1 = new TFile("/Volumes/shokair@nu.hep.upenn.edu/ntuples/sort/atm_salt_leta_sort.root");
  ...
  h6= new TH1F("h6","Delta t",100,0,.5);
  ...
  for (Int_t i=0;i<nd;i++){
    ...
    */  
    ...
    if(itr<itrmax && itr>itrmin && beta14>betamin && beta14<betamax ){
      hlcFlag=1;
    }
    else{hlcFlag=0;}
    if(rf<OV && ene>etrig && damnFlag==1 && hlcFlag==1 && nhits<nhitmax){
      cleanFlag=1; 
      numClean++;
      //cerr<<dt0<<endl;
	...
	if(itr>itrmin && itr<itrmax && beta14>betamin && beta14<betamax ){
	  hlcFlag=1;
	}
	else{hlcFlag=0;}
	if( rf<OV && ene>etrig && hlcFlag==1 && damnFlag2==1 &&nhits<nhitmax ){
	...
	if(mb==2){
	  for(int ct=0;ct<mb;ct++){
	    if(ct!= piNum){
	      dR=TMath::Sqrt((xi-xp[ct])*(xi-xp[ct])+(yi-yp[ct])*(yi-yp[ct])+(zi-zp[ct])*(zi-zp[ct]));
			h1->Fill(dR);
	    }
	  }
	...
	if(mb==3){
	  for(int ct=0;ct<mb;ct++){
	    if(ct!= piNum){
	      dR3[ct]=TMath::Sqrt((xi-xp[ct])*(xi-xp[ct])+(yi-yp[ct])*(yi-yp[ct])+(zi-zp[ct])*(zi-zp[ct]));
			h1->Fill(dR3[ct]);
	    }
	    else{
	      dR3[ct]=0;
	  ...
	  if(dR3[0]<distanceWindow && dR3[1]<distanceWindow && dR3[2]<distanceWindow){
	    num3fold++;
	    p->Fill(PeDr[coinNum-1][0],PeDt[coinNum-1][0]);
