...
{ 
	...
	TH2F * p= new TH2F("p"," Predessor Time Separation vs Predecessor Space Separation",200,0,500,50,0,.050);
			...
			if (cleanFlag1==1 ){
				...
				if(checkR==1){
					...
					if (numPred[numCoin]==0) {
						...
						pe1ID=predID[1];
						//twoFlag=1;
						r1=TMath::Sqrt(predX[0]*predX[0]+predY[0]*predY[0]+predZ[0]*predZ[0]);
						r2=TMath::Sqrt(predX[1]*predX[1]+predY[1]*predY[1]+predZ[1]*predZ[1]);
						if (deltaR[0]<distanceWindow) {
							check2[numCoin]=1;
							twoFlag=1;		
							/*
							cerr<<"2-Fold"<<endl;
							cerr<<predID[0]<<" "<<predEn[0]<<" "<<evTime[0]<<" "<<r1<<endl;
							cerr<<predID[1]<<" "<<predEn[1]<<" "<<evTime[1]<<" "<<r2<<endl;
							 */
							h4->Fill(predID[0]);
							h4->Fill(predID[1]);
							h2->Fill(predEn[0]);
						...
						r1=TMath::Sqrt(predX[0]*predX[0]+predY[0]*predY[0]+predZ[0]*predZ[0]);
						r2=TMath::Sqrt(predX[1]*predX[1]+predY[1]*predY[1]+predZ[1]*predZ[1]);
						r3=TMath::Sqrt(predX[2]*predX[2]+predY[2]*predY[2]+predZ[2]*predZ[2]);
