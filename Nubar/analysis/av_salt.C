 ...
 *  Modified 6/2/11
 ...
 8/23/11 no longer excluded av events
 8/24/11 Give Distribution of delta r
 */
{
	...
	TH1F * h8= new TH1F("h8","",400,0,400);
	TH1F * h9= new TH1F("h9","Acceptance Ratios ",400,0,400);
	TH1F * h10= new TH1F("h10","Acceptance Ratios ",400,0,400);
	TH1F * h11= new TH1F("h11","time",300,0,3);
	TH1F * h12= new TH1F("h12","DeltaR of Any Coincidence [Salt Phase]",1300,0,1300);
	TH1F * h13= new TH1F("h13","Time Differences Salt",300,0,.1);
	TH1F * h14= new TH1F("h14","1-3 Time Differences",300,0,.1);
	TH1F * h15= new TH1F("h15","2-3 Time Differences",300,0,.1);
	h6->SetLineColor(5);
	h7->SetLineColor(6);
	h8->SetLineColor(9);
	h8->SetLineWidth(3);
    ...
    {	
		...
		for(Int_t j=0; j<nd; ++j){
			...
			else if(rf<outerVolume && damnflag==1){
				cutFlag=0;
				totalClean++;
			}
			else {
			...
			if (cutFlag==1) {
				...
				while(l<=j) { //lookback at events
					...
					if (numPred[nump]==0 && timediff!=10) {
						lastTimediff=timediff;
						h14->Fill(timediff);
					}
					if (numPred[nump]==1 && timediff !=10) {
						h15->Fill(-timediff+lastTimediff);
						h13->Fill(timediff);
					}

					if (timediff<timeWindow &&timediff!=0 &&damnflag2==1 && ene>ethresh &&rf<outerVolume) {
						if (numPred[nump]==0) {
							posEn=ene;
							...
							nEn=ene;
							//cerr<<timediff<< " s time diff"<<endl;
				...
				if(checkR==1 ){
					if (numPred[nump]!=0) {
						cerr<<numPred[nump]<<" pred(s) for event"<<tempEventID<<endl;
					...
					if (numPred[nump]==0) {
						...
						cerr<<"pred had id "<<eventID[nump][0]<<endl;
						...
						h6->Fill(deltaR[0]);
						//h12->Fill(deltaR[0]);
						cerr<<"delta r= "<<deltaR[0]<<" cm from ri ="<<ri<<" to rf= "<<predR[0]<<endl;
						//from ("<<predX[0]<<",";
						//cerr<<predY[0]<<","<<predZ[0]<<") to ("<<xi<<","<<yi<<","<<zi<<")"<<endl;
						coin_events[nump][0]++;
						total2fold++;
						
					}
					else if (numPred[nump]==2){
						cerr<<"preds had ids "<<eventID[nump][0]<<" and "<<eventID[nump][1]<<endl;
						threeflag=1;
						twoflag=0;
						deltaR[0]=TMath::Sqrt((predX[0]-xi)*(predX[0]-xi)+(predY[0]-yi)*(predY[0]-yi)+(predZ[0]-zi)*(predZ[0]-zi));
				...
				if (nump>2) {
					if(check2[nump-2]==1 &&threeflag==0){
			...
			/*
			if(nump>1){
				if (coin_events[nump-1][0]!=0 ) {
					
					
					cerr<<"event "<<tid<<" had " <<coin_events[nump-1][0]<<" predecessors";
					if(coin_events[nump-1][0]==1){
						cerr<<": "<<eventID[nump-1][0]<<endl;
					}
					else if (coin_events[nump-1][0]==2){
						real3fold++;
						cerr<<": "<<eventID[nump-1][0]<<" and "<<eventID[nump-1][1]<<" for "<<real3fold<<" 3fold coincedences "<<endl;
		...
		cerr<<total3fold<<" real 3fold events and "<<total2fold<< " 2fold evens"<<endl;
		/*
		 ...
		 */
		cerr<<totalClean<<" total clean events"<<endl;
		//h8->Draw();
		//h9->Draw("same");
		//h10->Draw("same");
		std::stringstream cap;
		cap<<"Acceptance Ratios for Coincidences in Salt ["<<total2fold+total3fold<<" MC coincidences total]";
		/*
		TString st=cap.str();
		...
		leg2->AddEntry(h13,"1-2","l");
		leg2->AddEntry(h14,"1-3","l");
		leg2->AddEntry(h15,"2-3","l");
		leg2->Draw();
		TCanvas c3;
		h12->Draw();
		 */
