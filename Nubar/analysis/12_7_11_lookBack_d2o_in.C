...
{ 
	...
	TH1F * h5= new TH1F("h5","Energy",100,0,10);
	TH1F * h6= new TH1F("h6","Radial Position",650,0,650);
	TH1F * h7= new TH1F("h7","Nhits",100,0,100);
    ...
    {	
		...
		Td->SetBranchAddress("Enerau",&erau);
			...
			if (cleanFlag1==1){
				...
				while (l<j) {
					...
					if (timediff<timeWindow && timediff!=0) {
						...
						if (rf<fiducialVolume && ene>ethresh &&ene<emax) {
				...
				if(checkR==1){
					...
					if (coin_events[numCoin]==2) {
						threeFlag=1;
						twoFlag=0;
					}
					else if(coin_events[numCoin]==1) {
						twoFlag=1;
						threeFlag=0;
					}
				...
				if (numCoin>0) {					
					if(threeFlag==1 && twoFlag==0){
					...
					else if (threeFlag==0 && check2[numCoin-1]==1) {
						numTwoFold++;
						h2->Fill(tempEn[ct-1]);
						h3->Fill(tempEn[ct-2]);
						h4->Fill(tempID[ct-1]);
						h4->Fill(tempID[ct-2]);
						cerr<<"Two fold"<<endl;
						cerr<<tempID[ct-1]<<" "<<tempTime[ct-1]<<" "<<tempEn[ct-1]<<endl;
						cerr<<tempID[ct-2]<<" "<<tempTime[ct-2]<<" "<<tempEn[ct-2]<<endl;
