    ...
    {	
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (nhits>nhitCut && nhits<nhitMax && rf>0&& damnflag==1 &&ene>ethresh) {
				...
				for (Int_t st=0; st<numstep; st++) {
					isSecond[st]=0;
				}

			}
			if (rf<600 &&timediff<timeWindow) {
				//cerr<<"particle "<<evid<<" made it into the AV from start "<<rg<<" to "<<rf<<timediff<<" s after pred"<<endl;
				//cerr<<"the previous particle was at rf "<<trf<<" generated at "<<rg_i<<endl;
			}
			//look for a vertex if cuts are passes and vertex flag has not been set
			for (Int_t st=0; st<numstep; st++) {
				fV=outerVolume-rstep*st;
				if (cutFlag==1 && rf<fV) {
					if (TMath::Abs(timediff)<timeWindow && isSecond[st]==0 &&ene>emin) {
						//h1->Fill(tempTime,trf);
						
						//h1->Fill(totalTime,rf);
						isSecond[st]=1;
						if (st==0) {
							if (rf<fiducialVolume) {
								cerr<<"Evid "<<tid<<" Time: "<<tempTime<<" rf: "<<trf<<" generated at "<<rg_i<<endl;
								cerr<<"Evid "<<evid<<" Time: "<<totalTime<<" rf: "<<rf<<" generated at "<<rg<<" "<<timediff<< " s later"<<endl;
								cerr<<"Neutron energy is  "<<ene<<" MeV and had "<<nhits<<" nhits"<<endl;
							}
							h7->Fill(ene);
							h8->Fill(epos);
							h10->Fill(nhits);
						...
						if (TMath::Abs(timediff)>timeWindow && isSecond[st]==0) {
							//cerr<<evid<<" had time diff "<<totalTime-tempTime<<endl;
						}
						else if (TMath::Abs(timediff)>timeWindow && isSecond[st]==1) {
							//cerr<<evid<<" had time diff "<<totalTime-tempTime<<endl;
							isSecond[st]=0;
						}
