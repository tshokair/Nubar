//#include <string>
{
	...
	//files[0]="sno_012167_p2.root";
	files[0]="sno_012500_p3.root";	
	...
	for (Int_t k=0; k<numfiles;++k)
    {
		...
		Td->SetBranchAddress("Time_us",&ut2); //time in microseconds
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if(Int_t(day_diff)==1){
				...
				if(Int_t(dt1)==0){
					...
					dt3=(1-ut3_i/1.E9)+ut3/1.E9;
					timediff=dt2;
					//cerr<<timediff<<" t difference "<<dt1<<" s diff " <<dt2<<" us diff "<<dt3<<" ns diff"<<endl;
			...
			numRawEvents++;
			if (nhits>27.5 &&nhits<150 &&rmag<550&&day_diff==0 &&thetaij>.75 &&thetaij<1.45&&itr<.95&&itr>.55 &&damnflag==1){
				numev++;
				h1->Fill(nhits);
				h10->Fill(enep);
				h4->Fill(ut1);
				 ...
				 */
				if((isFirst==0 && isSecond==0)||isThird==1||rfit>350||timediff>.15){
					tempEventID=evid;
					temp_pos=enep;
					ut1_i=ut1;
					ut2_i=ut2;
					...
					h14->Fill(rmag);
					h8->Fill(nhits);
					//cerr<<"time diff from last event "<<timediff<<endl; 
					jdy_i=jdy;
				}
				else if ( /*is_pos==0 &&*/ rfit<500 &&timediff<=0.150){
			...
			} //events within high level cuts
			else {
				jdy_i=jdy;
				//isFirst=0;
				//isSecond=0;
			} //if event does not make it past high level cuts
		}//end loop over files
