...
{ 
	...
	ofstream tCoins;
	tCoins.open("tCoinsSin.txt");
	ifstream runList;
	//runList.open("d2o_data.runlist");
	//runList.open("d2o_partial.runlist");
	runList.open("testSalt.txt");
	//runList.open("dTest.txt");
	numfiles =0;
	while(!runList.eof()){
		...
		for(Int_t j=0; j<nd; ++j){
			...
			if (cleanFlag1==1 && Int_t(enu)==12){
				...
				while (l<=lbEnd) {
					...
					if (TMath::Abs(timediff)<timeWindow && Int_t(enu)==12) {
